...
def render_env_output(rows, source_site: str) -> str:
    ...
    return "\n".join([
        f"METADATA_QUEUE_JSON={shlex.quote(payload)}",
        f"METADATA_QUEUE_COUNT={len(rows)}",
        f"METADATA_SOURCE_SITE={shlex.quote(source_site)}",
    ])


def main():
    parser = argparse.ArgumentParser(description='Pick concrete metadata rows for detail-only metadata patching.')
